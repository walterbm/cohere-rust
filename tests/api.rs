...
use cohere_rust::{
    Cohere,
    api::{
        EmbedInputType, EmbedModel, Truncate,
...
async fn test_embed() {
    ...
    let mock_endpoint = mock_server
            ...
            .with_header("content-type", "application/json")
            .with_body(
                format!("{{\"id\":\"3915734e-bd8e-4ff4-9be1-f6dfd29dc386\",\"texts\":[\"hi\"],\"embeddings\":{{\"float\":[{}]}},\"meta\":{{\"api_version\":{{\"version\":\"1\"}}}}}}", serde_json::to_string(&mock_embeddings).unwrap()),
            )
            .create_async()
            .await;
...
async fn test_api_failure() {
    ...
    assert_eq!(
        "API request failed with status code `500 Internal Server Error` and error message `invalid request: inputs cannot be empty`",
        response.to_string()
    );
}
...
async fn test_streaming_api_error() {
    ...
    assert_eq!(
        "API request failed with status code `429 Too Many Requests` and error message `You are using a Trial key, which is limited to 10 API calls / minute.`",
        response.to_string()
    );
}

#[tokio::test]
async fn test_chat_v2_non_stream() {
    ...
    let request = ChatV2Request {
        ...
        messages: vec![ChatV2Message::user("What is 2 + 2?")],
        ..ChatV2Request::new("command-a-03-2025", "")
    };

    let response = client.chat_v2_non_stream(&request).await;
    ...
    assert_eq!("COMPLETE", response.finish_reason);
    assert_eq!(Some("The answer is 4."), response.message.text());
    assert!(response.message.tool_calls.is_empty());
}

#[tokio::test]
async fn test_chat_v2_stream() {
    use cohere_rust::api::chat_v2::ChatV2Request;

    let mut mock_server = mockito::Server::new_async().await;
    let mock_url = mock_server.url();

    let sse_body = [
        "data: {\"type\":\"message-start\",\"id\":\"msg123\"}\n\n",
    ...
    let request = ChatV2Request::new("command-a-03-2025", "Say hello");
    let response = client.chat_v2(&request).await;
...
#[tokio::test]
async fn test_client_debug_clone() {
    ...
    assert!(debug_str.contains("Cohere"));
